...
class CitySelector extends window.HTMLElement {
  constructor() {
    ...
    this.supportsDatalist =
      'list' in document.createElement('input') && !!(document.createElement('datalist') && window.HTMLDataListElement)
  ...
  connectedCallback() {
    ...
    if (!this.supportsDatalist) {
      window
        .fetch(`${config.BASE_URL}/api/capitals`)
        .then(res => res.json())
        .then(json => {
  ...
  async search(str) {
    let res = await window.fetch(`${config.BASE_URL}/api/cities/?q=${str}`)
    res = await res.json()
    return res.cities
  }
  _updateRendering() {
